...
export function fixStationName(name: string) {
    ...
    return name.split('Platform')[0].split('Stall')[0].replace('Transit Station', '').replace('Station', '').replace('Metrolink', '').replace('Amtrak', '').replace('Transportation Center', '').replace('Transit Center', '').replace('Transit Ctr', '').trim()
}