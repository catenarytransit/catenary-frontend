...
<div class="mt-1 px-2 py-2 mb-1">
    ...
    {#if agency_statement}
    <p class="leading-none text-sm lg:text-base md:leading-tight text-gray-700 italic dark:text-gray-400">{agency_statement}</p>
    {/if}

    <ul class="list-disc">
    {#each native_land.links as {title, url}}
        <li>
            <a href={url} target="_blank" rel="noopener" class="underline text-blue-700 dark:text-blue-400">{title}</a>
        </li>
{/each}
</ul>
</div>
