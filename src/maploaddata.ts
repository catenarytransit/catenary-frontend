export function determineFeeds(map:any, staticfeeds:any, operators:any, realtimefeeds:any,geolocation:GeolocationPosition) {
 ...
 //  console.log('static data',static_data)
    ...
    const operators_in_frame = [... new Set(static_data.map((f:any) => f.operators).flat().filter((x:any) => x != undefined))];
   ...
   // console.log('operators_data',operators_data)
    ...
    static_data.forEach((x:any) => {static_data_obj[x.onestop_feed_id] = x; })

    let operators_data_obj:any = {};

    operators_data.forEach((x:any) => {operators_data_obj[x.onestop_operator_id] = x; })
    ...
    Object.values(operators_data_obj).forEach((eachoperator:any) => {
        ...
        eachoperator.gtfs_realtime_feeds.forEach((eachrealtimefeedid:any) => {
            ...
            if (realtime_feeds_data_obj[eachrealtimefeedid] === undefined) {
                ...
                realtime_feeds_data_obj[eachrealtimefeedid].operators_to_gtfs_ids[eachoperator.onestop_operator_id] = eachoperator.realtime_onestop_feeds_to_gtfs_ids[eachrealtimefeedid];
