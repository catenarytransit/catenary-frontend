    ...
    function fetchTime() {
        ...
        .then((body) => {
           ...
           if (body !== null) {
            ...
            let result =  (serverClientRequestDiffTime - fetchstart + fetchend - serverClientResponseDiffTime )/2;
...
<p class=""><span class="font-bold">Client ISO8601</span> <span class="font-mono">{new Date(current_time_client_ns / 1e6)}</span>
  <p><span class="font-bold">Client Unix Time</span> <span  class="font-mono">{(current_time_client_ns / 1e9).toFixed(3)}</span></p>
